# BR Port — prompt para a próxima conversa (a Dona Cida quadrada v3: o aceite e o passo seguinte)
...
trabalhador do rodapé — e **a cor também: Standard nos retratos** (os props
continuam no AgX). O caminho até aqui, em `art_lab/retratos/cida_seria/`: a
redonda `v1` (rejeitada), a redonda `v2` (não escolhida), a `quadrada_v1`
(escolhida contra a v2), a `quadrada_v2` (ajustada: cabelo, óculos, tronco,
rosto) e a **`quadrada_v3`**, que é a candidata: cabelo repartido ao meio com
a calote no tom de sombra por baixo das mechas, sobrancelhas em arco
castanhas, nariz em cunha, lábio de cor, óculos oitavados finos, tronco de 12
lados com ombro, carcela e bolso — em **Standard a −0,35 EV**. Fotografada no
jogo numa cópia: **muda 1 foto em 31** (o `boletim`), só na caixa do retrato.
**Nada disto está em `brport_vs/`**: o retrato do jogo continua o de
...
**Pede o veredito do Bruno sobre a `quadrada_v3`, se ele ainda não o deu, e
age conforme ele.** Mostre-lhe a `quadrada_v3/prancha_hoje_q2_q3.png` e a
`quadrada_v3/jogo_boletim_hoje_vs_q3.png`, e leia antes o README dela.
...
isso foi dito antes de modelar. ⚠️ E quando ele escolhe «ajustar» sem dizer
o quê, pergunte O QUÊ com a lista do que você vê de fraco — foi assim nas
duas voltas desta conversa, e ele marcou tudo nas duas.

- **Se ele pedir mudanças:** `quadrada_v4/`, pelo script da v3 (`PREVIA=1`
...
- **Se aceitar na foto do jogo**, é a integração — e ela é maior do que um
  ...
  1. a decisão **`055`** em `docs/decisoes/` (o modelo quadrado fica; porquê;
     o Standard a −0,35 EV nos retratos; o que se mediu), antes do código;
  ...
  4. o Standard e a exposição valem para os NOVE retratos (é o estúdio
     deles), logo os nove regeram-se; o `#3b2513` (o degrau abaixo do cabelo)
     entra na `PALETA`. Suítes, a bateria inteira, as folhas de contato;
...
- **O veredito da `quadrada_v3`** (§2).
- **AgX ou Standard nos PROPS** (plano de arte §7.1): nos retratos ele escolheu
  Standard; o pipeline dos props continua a renderizar pelo AgX sem o
  declarar (nenhum prop passa de ~190 no p99). Contorno por casco invertido
  (P5) também espera por ele.
...
- ⚠️ **O Standard pede a exposição**: a 0 EV, 54% dos pixels claros da gola e
  do olho saíam a 255; a −0,35 EV, zero. Meça o estouro a cada mudança de cor.
