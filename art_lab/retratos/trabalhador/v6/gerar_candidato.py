"""O trabalhador do rodapé em BUSTO no kit afinado — a candidata v6.
...
`CATALOGO` de `brp_porto.py` continua a exportar o boneco de corpo inteiro.
    ...
    python3.11 art_lab/retratos/trabalhador/v6/gerar_candidato.py
...
# A CABEÇA A 56% DO QUADRO, e não aos 60% dos três que falam: ele identifica-se
# pelo COLETE, e a 60% a faixa de baixo saía cortada pelo quadro (v4).
CABECA = 0.56
...
def montar(M, est):
    ...
    brp_retratos.enquadrar(est.cena, NOME, pecas, medir, cabeca=CABECA)
