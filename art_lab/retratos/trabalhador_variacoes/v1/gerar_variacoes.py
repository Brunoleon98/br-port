"""As variações do trabalhador do rodapé — a leva v1.
...
`trabalhador_retrato` do `CATALOGO` continua sem perfil, que é o padrão.
    ...
    python3.11 art_lab/retratos/trabalhador_variacoes/v1/gerar_variacoes.py [nome ...]
...
# é o de cara lisa, e nunca no padrão, que tem de sair igual.
_PELOS = {("adulto", "branca"): "bigode", ("adulto", "preta"): "cavanhaque",
          ("veterano", "parda"): "bigode", ("veterano", "branca"): "cavanhaque",
          ("veterano", "preta"): "bigode"}
...
def _perfis():
    ...
    for sexo in br.SEXOS:
        for idade in br.IDADES:
            for cor in br.CORES:
                pelos = _PELOS.get((idade, cor)) if sexo == "homem" else None
                tabela["trabalhador_%s_%s_%s" % (sexo, idade, cor)] = dict(
                    sexo=sexo, idade=idade, cor=cor, pelos=pelos)
    return tabela
