...
# Branching foliage at far left and right makes this a group, not repeated shrubs.
for x,y,sc,seed in [(27,115,13,1),(47,98,10,2),(59,126,12,3),(257,119,12,4),(276,112,8,5)]:
...
add('</svg>')
svg='\n'.join(parts)
(OUT/'vegetacao_grupo_f1_v1.svg').write_text(svg,encoding='utf8')
subprocess.run(['inkscape',str(OUT/'vegetacao_grupo_f1_v1.svg'),'--export-filename='+str(OUT/'vegetacao_grupo_f1_v1.png')],check=True,stdout=subprocess.DEVNULL)
# Asset origin is the bounding transparent canvas, 300x190 logical units.
# Preview composites are separate SVG documents, never modifications to source maps.
def preview(file,dx,dy,w,h):
 map_png=Path('art_lab/f01/13a/v5/mapa_v5.png').resolve()
 asset_png=(OUT/'vegetacao_grupo_f1_v1.png').resolve()
 b1=base64.b64encode(map_png.read_bytes()).decode('ascii')
 b2=base64.b64encode(asset_png.read_bytes()).decode('ascii')
 doc=f'''<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="720" height="720" viewBox="0 0 720 720"><image xlink:href="data:image/png;base64,{b1}" width="720" height="720"/><image xlink:href="data:image/png;base64,{b2}" x="{dx}" y="{dy}" width="{w}" height="{h}"/></svg>'''
 p=OUT/(file+'.svg');p.write_text(doc)
 subprocess.run(['inkscape',str(p),'--export-filename='+str(OUT/(file+'.png'))],check=True,stdout=subprocess.DEVNULL)
 p.unlink()
preview('mapa_estatico_v1',12,10,100,63)
print('written',OUT)
